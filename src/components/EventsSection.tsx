...
export default function EventsSection() {
  ...
  useEffect(() => {
    // Auto-rotate events every 5 seconds on all devices
  ...
  return (
    <section className="relative overflow-hidden py-20" id="events">
      ...
      {/* Decorative elements */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#FF00FF] via-[#950713] to-[#FFFF00]"></div>
      <div className="absolute top-0 right-0 w-64 h-64 bg-[#FF00FF] rounded-full filter blur-[100px] opacity-20"></div>
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#00FFFF] rounded-full filter blur-[120px] opacity-20"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                {events.map((_, index) => (
                  <button 
                    ...
                    className={`w-3 h-3 rounded-full transition-all duration-300 ${
                      index === activeEvent ? 'bg-[#FF00FF] scale-125' : 'bg-[#00FFFF]/30 hover:bg-[#FFFF00]'
                    }`}
        ...
        {/* Mobile event cards - using same slider as desktop for consistency */}
        <div className="md:hidden mb-8">
          <div className="relative mb-6">
            <div className="relative h-[300px] overflow-hidden rounded-xl shadow-2xl transform transition-all duration-500">
              {events.map((event, index) => (
                <div 
                  key={event.id}
                  className={`absolute inset-0 transition-opacity duration-1000 ${
